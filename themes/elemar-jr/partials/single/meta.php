...
if ( empty( $sections )
	|| ( 'post' !== get_post_type() && 'archive' !== get_post_type()) ) {
	return;
}
...
<ul class="meta clr">
	...
	foreach ( $sections as $section ) { ?>
		...
		if ( 'categories' == $section ) { ?>
			<li class="meta-cat"><i class="icon-folder" aria-hidden="true"></i><?php
			if ( 'archive' === get_post_type() ) {
				do_action( 'the_taxonomy_archive_category' );
			} else {
				the_category( ' <span class="owp-sep">/</span> ', get_the_ID() );
			}
		?></li>
		<?php } ?>
